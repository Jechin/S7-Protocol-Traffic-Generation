...
Description: 
Autor: Jechin
Usage: ./astf-sim -f ~/s7/astf/s7_ASTF_Upload.py --full -o ~/s7/astf/pcap/s7_ASTF_Upload.pcap -t config="config_Upload
'''
...
def check_config_upload(config: dict):
    if "Start" not in config.keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'Start' key", color="red")
        sys.exit(1)
    if "uploadID" not in config["Start"].keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'uploadID' key in 'Start'", color="red")
        sys.exit(1)
    if "filename_length" not in config["Start"].keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'filename_length' key in 'Start'", color="red")
        sys.exit(1)
    if "filename" not in config["Start"].keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'filename' key in 'Start'", color="red")
        sys.exit(1)

    if "Upload" not in config.keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'Upload' key", color="red")
        sys.exit(1)
    if type(config["Upload"]) is not list:
        colormsg(title_with_color="[Error]", msg="config file error: 'Upload' must be a list", color="red")
        sys.exit(1)
    for item in config["Upload"]:
        if "length" not in item.keys():
            colormsg(title_with_color="[Error]", msg="config file error: no 'data_length' key in 'Upload'", color="red")
            sys.exit(1)
        if "data" not in item.keys():
            colormsg(title_with_color="[Error]", msg="config file error: no 'data' key in 'Upload'", color="red")
            sys.exit(1)
...
def check_config(config: dict):
    ...
    if "Upload" not in config.keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'Upload' key", color="red")
        sys.exit(1)
    check_config_upload(config["Upload"])
...
class Prof1():
    ...
    def create_profile(self, config):
        ...
        job_upload_obj = s7.Job_Determine_Function(function=JobFunction.UPLOAD, args=config["Upload"])
        ack_upload_obj = s7.ACK_Data_Determine_Function(function=JobFunction.UPLOAD, args=config["Upload"])
    ...
    def get_profile(self, tunables, **kwargs):
        ...
        parser.add_argument('--config', type=str, default='config_Upload.json', help='path of config file')
