...
Description: 
Autor: Jechin
Usage: ./astf-sim -f ~/s7/astf/s7_ASTF_PLC_Stop.py --full -o ~/s7/astf/pcap/s7_ASTF_PLC_Stop.pcap -t config="config_PLC_Stop.json"
'''
...
def check_config_Stop(config: dict):
...
def check_config(config: dict):
    ...
    if "PLC_Stop" not in config.keys():
        colormsg(title_with_color="[Error]", msg="config file error: no 'Stop' key", color="red")
        sys.exit(1)
    check_config_Stop(config["PLC_Stop"])
...
class Prof1():
    ...
    def get_profile(self, tunables, **kwargs):
        ...
        parser.add_argument('--config', type=str, default='config_PLC_Stop.json', help='path of config file')
